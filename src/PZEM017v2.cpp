/*
* Copyright (c) 2023 Chung Ta (IT Cinet https://www.cinet.asia/) @thelightings 
 * Copyright (c) 2020 Maxz Maxzerker (for PZEM-017) https://github.com/maxzerker
 * Copyright (c) 2019 Jakub Mandula https://github.com/mandulaj
*/


#include "PZEM017v2.h"
#include <stdio.h>

#define REG_VOLTAGE     0x0000
#define REG_CURRENT     0x0001  
#define REG_POWER_L     0x0002
#define REG_POWER_H     0x0003
#define REG_ENERGY_L    0x0004
#define REG_ENERGY_H    0x0005
#define REG_HVALARM     0x0006
#define REG_LVALARM     0x0007

#define CMD_RHR         0x03  
#define CMD_RIR         0X04
#define CMD_WSR         0x06
#define CMD_CAL         0x41
#define CMD_REST        0x42

#define WREG_ALARM_THR_HV   0x0000
#define WREG_ALARM_THR_LV   0x0001
#define WREG_ADDR           0x0002
#define WREG_SADDR          0x0003 

#define SHUNT_100A      0x0000
#define SHUNT_50A       0x0001  
#define SHUNT_200A      0x0002
#define SHUNT_300A      0x0003

#define UPDATE_TIME     200

#define RESPONSE_SIZE 32
#define READ_TIMEOUT 100
#define PZEM_BAUD_RATE 9600 

#define INVALID_ADDRESS 0x00

#define DEBUG

// Debugging function;
void printBuf(uint8_t* buffer, uint16_t len){
#ifdef DEBUG
    for(uint16_t i = 0; i < len; i++){
        char temp[6];
        sprintf(temp, "%.2x ", buffer[i]);
        Serial.print(temp);

    }
    Serial.println();
#endif
}


#if defined(PZEM017v2_SOFTSERIAL)
/*!
 * PZEM017v2::PZEM017v2
 *
 * Software Serial constructor (WARNING: Will be deprecated)
 *
 * @param receivePin RX pin
 * @param transmitPin TX pin
 * @param addr Slave address of device
*/
PZEM017v2::PZEM017v2(uint8_t receivePin, uint8_t transmitPin, uint8_t addr)
{
    localSWserial = new SoftwareSerial(receivePin, transmitPin); // We will need to clean up in destructor
    localSWserial->begin(PZEM_BAUD_RATE);

    init((Stream *)localSWserial, true, addr);
}

/*!
 * PZEM017v2::PZEM017v2
 *
 * Software Serial constructor for SoftwareSerial instance
 *
 * @param port Software serial port instance
 * @param addr Slave address of device
*/
PZEM017v2::PZEM017v2(SoftwareSerial& port, uint8_t addr)
{
    port.begin(PZEM_BAUD_RATE);
    init((Stream *)&port, true, addr);
}

/*!
 * PZEM017v2::PZEM017v2
 *
 * Software Serial constructor for Stream instance
 *
 * @param port Stream instance
 * @param addr Slave address of device
*/
PZEM017v2::PZEM017v2(Stream& port, uint8_t addr)
{
    init(&port, true, addr);
}

#endif

/*!
 * PZEM017v2::PZEM017v2
 *
 * Hardware serial constructor
 *
 * @param port Hardware serial to use
 * @param receivePin (Only ESP32) receive Pin to use
 * @param transmitPin (Only ESP32) transmit Pin to use
 * @param addr Slave address of device
*/
#if defined(ESP32)

PZEM017v2::PZEM017v2(HardwareSerial& port, uint8_t receivePin, uint8_t transmitPin, uint8_t addr)
{
    port.begin(PZEM_BAUD_RATE, SERIAL_8N1, receivePin, transmitPin);
    init((Stream *)&port, false, addr);
}
#else


PZEM017v2::PZEM017v2(HardwareSerial& port, uint8_t addr)
{
    port.begin(PZEM_BAUD_RATE);
    init((Stream *)&port, false, addr);
}
#endif

/*!
 * PZEM017v2::~PZEM017v2
 *
 * Destructor deleting software serial
 *
*/
PZEM017v2::~PZEM017v2()
{
    // TODO: Remove local SW serial
    // This is not the correct way to do it. 
    // Best solution would be to completely remove local SW serial instance and not deal with it.
#if defined(PZEM017v2_SOFTSERIAL)
    if(this->localSWserial != nullptr){
        delete this->localSWserial;
    }
#endif
}

/*!
 * PZEM017v2::voltage
 *
 * Get line voltage in Volts
 *
 * @return current L-N volage
*/
float PZEM017v2::voltage()
{
    if(!updateValues()) // Update vales if necessary
        return NAN; // Update did not work, return NAN

    return _currentValues.voltage;
}

/*!
 * PZEM017v2::current
 *
 * Get line in Amps
 *
 * @return line current
*/
float PZEM017v2::current()
{
    if(!updateValues())// Update vales if necessary
        return NAN; // Update did not work, return NAN

    return _currentValues.current;
}

/*!
 * PZEM017v2::power
 *
 * Get Active power in W
 *
 * @return active power in W
*/
float PZEM017v2::power()
{
    if(!updateValues()) // Update vales if necessary
        return NAN; // Update did not work, return NAN

    return _currentValues.power;
}

/*!
 * PZEM017v2::energy
 *
 * Get Active energy in kWh since last reset
 *
 * @return active energy in kWh
*/
float PZEM017v2::energy()
{
    if(!updateValues()) // Update vales if necessary
        return NAN; // Update did not work, return NAN

    return _currentValues.energy;
}

bool PZEM017v2::voltage_alarm_high()
{
    if(_currentValues.alarms_hv==0xFFFF)
        return true;

    return false;
}

/*!
 * PZEM017v2l::pf
 *
 * Get power factor of load
 *
 * @return load power factor
*/
bool PZEM017v2::voltage_alarm_low()
{
    if(_currentValues.alarms_lv==0xFFFF)
        return true;

    return false;
}
void printHex(int num, int precision) {
     char tmp[16];
     char format[128];

     sprintf(format, "%%.%dX", precision);

     sprintf(tmp, format, num);
     Serial.println(tmp);
}

/*!
 * PZEM017v2::frequency
 *
 * Get current line frequency in Hz
 *
 * @return line frequency in Hz

float PZEM017v2::frequency()
{
    if(!updateValues()) // Update vales if necessary
        return NAN; // Update did not work, return NAN

    return _currentValues.frequency;
}
*/

/*!
 * PZEM017v2::pf
 *
 * Get power factor of load
 *
 * @return load power factor

float PZEM017v2::pf()
{
    if(!updateValues()) // Update vales if necessary
        return NAN; // Update did not work, return NAN

    return _currentValues.pf;
}
*/

/*!
 * PZEM017v2::sendCmd8
 *
 * Prepares the 8 byte command buffer and sends
 *
 * @param[in] cmd - Command to send (position 1)
 * @param[in] rAddr - Register address (postion 2-3)
 * @param[in] val - Register value to write (positon 4-5)
 * @param[in] check - perform a simple read check after write
 *
 * @return success
*/
bool PZEM017v2::sendCmd8(uint8_t cmd, uint16_t rAddr, uint16_t val, bool check, uint16_t slave_addr){
    uint8_t sendBuffer[8]; // Send buffer
    uint8_t respBuffer[8]; // Response buffer (only used when check is true)

    if((slave_addr == 0xFFFF) ||
       (slave_addr < 0x01) ||
       (slave_addr > 0xF7)){
        slave_addr = _addr;
    }

    sendBuffer[0] = slave_addr;                   // Set slave address
    sendBuffer[1] = cmd;                     // Set command

    sendBuffer[2] = (rAddr >> 8) & 0xFF;     // Set high byte of register address
    sendBuffer[3] = (rAddr) & 0xFF;          // Set low byte =//=

    sendBuffer[4] = (val >> 8) & 0xFF;       // Set high byte of register value
    sendBuffer[5] = (val) & 0xFF;            // Set low byte =//=

    setCRC(sendBuffer, 8);                   // Set CRC of frame

    _serial->write(sendBuffer, 8); // send frame

    if(check) {
        if(!receive(respBuffer, 8)){ // if check enabled, read the response
            return false;
        }

        // Check if response is same as send
        for(uint8_t i = 0; i < 8; i++){
            if(sendBuffer[i] != respBuffer[i])
                return false;
        }
    }
    return true;
}

bool PZEM017v1::getParameters()
{
    static uint8_t response[13];

    // If we read before the update time limit, do not update
    if(_lastHoldingRead + UPDATE_TIME > millis()){
        return true;
    }

    // Read 3 registers starting at 0x00
    sendCmd8(CMD_RHR, 0x00, 0x04, false);

    if(recieve(response, 13) != 13){ // Something went wrong
        return false;
    }

    // Update the current paramaters
    _parameterValues.HVAlarmVoltage = ((uint32_t)response[3] << 8 | // Raw voltage in 0.01V
                                        (uint32_t)response[4])/100.0;

    _parameterValues.LVAlarmVoltage = ((uint32_t)response[5] << 8 | // Raw voltage in 0.01V
                                        (uint32_t)response[6])/100.0;

    _parameterValues.address =        ((uint32_t)response[7] << 8 | // Raw address 0x00-0xf7
                                        (uint32_t)response[8]);

    _parameterValues.shunttype =      ((uint32_t)response[9] << 8 | // Shunt type 0x0000 - 0x0003 (100A/50A/200A/300A)
                                        (uint32_t)response[10]);
    // Record current time as _lastHoldingRead
    _lastHoldingRead = millis();

    return true;
}

/*!
 * PZEM017v2::setAddress
 *
 * Set a new device address and update the device
 * WARNING - should be used to set up devices once.
 * Code initializtion will still have old address on next run!
 *
 * @param[in] addr New device address 0x01-0xF7
 *
 * @return success
*/
bool PZEM017v2::setAddress(uint8_t addr) 
{
    if(addr < 0x01 || addr > 0xF7) // sanity check
        return false;

    // Write the new address to the address register
    if(!sendCmd8(CMD_WSR, WREG_ADDR, addr, true))
        return false;

    _addr = addr; // If successful, update the current slave address

    return true;
}

bool PZEM017v2::setSAddress(uint8_t saddr) 
{
    if(saddr < 0x0000 || saddr > 0x0003) // sanity check
        return false;

    // Write the new address to the address register
    if(!sendCmd8(CMD_WSR, WREG_SADDR, saddr, true))
        return false;

    _saddr = saddr; // If successful, update the current slave address

    return true;
}


/*! 
 * PZEM017v2::readAddress()
 * 
 * Read address from the device memory
 * @return success
*/
uint8_t PZEM017v2::readAddress(bool update)
{
    static uint8_t response[7];
    uint8_t addr = 0;
    // Read 1 register
    if (!sendCmd8(CMD_RHR, WREG_ADDR, 0x01, false))
        return INVALID_ADDRESS;


    if(receive(response, 7) != 7){ // Something went wrong
        return INVALID_ADDRESS;
    }

    // Get the current address
    addr = ((uint32_t)response[3] << 8 | // Raw address
                              (uint32_t)response[4]);

    // Update the internal address if desired
    if(update){
        _addr = addr;
    }
    return addr;
}

uint8_t PZEM017v2::readSAddress(bool update)
{
    static uint8_t response[7];
    uint8_t saddr = 0;
    // Read 1 register
    if (!sendCmd8(CMD_RHR, WREG_SADDR, 0x0000, false))
        return INVALID_ADDRESS;


    if(receive(response, 7) != 7){ // Something went wrong
        return INVALID_ADDRESS;
    }

    // Get the current address
    saddr = ((uint32_t)response[3] << 8 | // Raw address
                              (uint32_t)response[4]);

    // Update the internal address if desired
    if(update){
        _saddr = saddr;
    }
    return saddr;
}


/*!
 * PZEM017v2::getAddress
 *
 * Get the current device address
 *
 * @return address
*/
uint8_t PZEM017v2::getAddress()
{
    return _addr;
}

uint8_t PZEM017v2::getSAddress()
{
    return _saddr;
}

/*!
 * PZEM017v2::setPowerAlarm
 *
 * Set power alarm threshold in watts
 *
 * @param[in] watts Alamr theshold
 *
 * @return success

bool PZEM017v2::setPowerAlarm(uint16_t watts)
{
    if (watts > 25000){ // Sanitych check
        watts = 25000;
    }

    // Write the watts threshold to the Alarm register
    if(!sendCmd8(CMD_WSR, WREG_ALARM_THR, watts, true))
        return false;

    return true;
}
*/

/*!
 * PZEM017v2::getPowerAlarm
 *
 * Is the power alarm set
 *
 *
 * @return arlam triggerd

bool PZEM017v2::getPowerAlarm()
{
    if(!updateValues()) // Update vales if necessary
        return NAN; // Update did not work, return NAN

    return _currentValues.alarms != 0x0000;
}
*/


bool PZEM017v2::setVoltageAlarm_HV(uint16_t volts)
{
    volts= volts*100;
    if (volts > 30000){ // Sanitych check
        volts = 30000;
    }
    Serial.println("setVoltageAlarm_HV");
    // Write the watts threshold to the Alarm register
    if(!sendCmd8(CMD_WSR, WREG_ALARM_THR_HV, volts, true))
    {
        Serial.println("Error:setVoltageAlarm_HV");
        return false;
    }

    return true;
}

bool PZEM017v2::getVoltageAlarm_HV()
{
    if(!updateValues()) // Update vales if necessary
        return NAN; // Update did not work, return NAN

    return _currentValues.alarms_hv != 0x0000;
}

bool PZEM017v2::setVoltageAlarm_LV(uint16_t volts)
{
    volts= volts*100;
    if (volts > 30000){ // Sanitych check
        volts = 30000;
    }
    Serial.println("setVoltageAlarm_LV");
    // Write the watts threshold to the Alarm register
    if(!sendCmd8(CMD_WSR, WREG_ALARM_THR_LV, volts, true))
    {
        Serial.println("Error:setVoltageAlarm_LV");
        return false;
    }
    return true;
}

bool PZEM017v2::getPowerAlarm_LV()
{
    if(!updateValues()) // Update vales if necessary
        return NAN; // Update did not work, return NAN

    return _currentValues.alarms_lv != 0x0000;
}

/*!
 * PZEM017v2::init
 *
 * initialization common to all consturctors
 *
 * @param[in] addr - device address
 *
 * @return success
*/
void PZEM017v2::init(Stream* port, bool isSoft, uint8_t addr){
    if(addr < 0x01 || addr > 0xF8) // Sanity check of address
        addr = PZEM_DEFAULT_ADDR;
    _addr = addr;

    this->_serial = port;
    this->_isSoft = isSoft;

    // Set initial lastRed time so that we read right away
    _lastRead = 0;
    _lastRead -= UPDATE_TIME;

    _isConnected = false; // We have not received anything yet...
}


/*!
 * PZEM017v2::updateValues
 *
 * Read all registers of device and update the local values
 *
 * @return success
*/
bool PZEM017v2::updateValues() 
{
    static uint8_t response[21];  /

    // If we read before the update time limit, do not update
    if(_lastRead + UPDATE_TIME > millis()){
        return true;
    }

    DEBUGLN("Updating Values");


    // Read 10 registers starting at 0x00 (no check)
    sendCmd8(CMD_RIR, 0x00, 0x08, false);

    if(recieve(response, 21) != 21){ // Something went wrong
        return false;
    }


    // Update the current values
    _currentValues.voltage = ((uint32_t)response[3] << 8 | // Raw voltage in 0.01V
                              (uint32_t)response[4])/100.0;

    _currentValues.current =  ((uint32_t)response[5] << 8 | // Raw voltage in 0.01A
                              (uint32_t)response[6])/100.0;

     _currentValues.power =   ((uint32_t)response[7] << 8 | // Raw power in 0.1W
                              (uint32_t)response[8] |
                              (uint32_t)response[9] << 24 |
                              (uint32_t)response[10] << 16) / 10.0;

     _currentValues.energy =  ((uint32_t)response[11] << 8 | // Raw Energy in 1Wh
                              (uint32_t)response[12] |
                              (uint32_t)response[13] << 24 |
                              (uint32_t)response[14] << 16) / 1000.0;

    _currentValues.alarms_hv =  ((uint32_t)response[15] << 8 | // Raw alarm value
                               (uint32_t)response[16]);

    _currentValues.alarms_lv = ((uint32_t)response[17] << 8 | // Raw alarm value
                               (uint32_t)response[18]);

    // Record current time as _lastRead
    _lastRead = millis();

    return true;
}


/*!
 * PZEM017v2::resetEnergy
 *
 * Reset the Energy counter on the device
 *
 * @return success
*/
bool PZEM017v2::resetEnergy(){
    uint8_t buffer[] = {0x00, CMD_REST, 0x00, 0x00};
    uint8_t reply[5];
    buffer[0] = _addr;

    setCRC(buffer, 4);
    _serial->write(buffer, 4);

    uint16_t length = receive(reply, 5);

    if(length == 0 || length == 5){
        return false;
    }

    return true;
}

/*!
 * PZEM017v2::receive
 *
 * Receive data from serial with buffer limit and timeout
 *
 * @param[out] resp Memory buffer to hold response. Must be at least `len` long
 * @param[in] len Max number of bytes to read
 *
 * @return number of bytes read
*/
uint16_t PZEM017v2::receive(uint8_t *resp, uint16_t len) 
{
      //* This has to only be enabled for Software serial
    #if (defined(PZEM017v2_SOFTSERIAL) && (defined(__AVR__)) || defined(ESP8266))
        if(_isSoft)
            ((SoftwareSerial *)_serial)->listen(); // Start software serial listen
    #endif
    unsigned long startTime = millis(); // Start time for Timeout
    uint8_t index = 0; // Bytes we have read
    while((index < len) && (millis() - startTime < READ_TIMEOUT))
    {
        if(_serial->available() > 0)
        {
            uint8_t c = (uint8_t)_serial->read();

            resp[index++] = c;
        }
        //yield();	// do background netw tasks while blocked for IO (prevents ESP watchdog trigger)
    }

    //Check CRC with the number of bytes read
    if(!checkCRC(resp, index)){
        _isConnected = false; // We are no longer connected
        Serial.println("CRC Error");
        return 0;
    }

     _isConnected = true; // We received a reply 
    return index;
}

/*!
 * PZEM017v2::checkCRC
 *
 * Performs CRC check of the buffer up to len-2 and compares check sum to last two bytes
 *
 * @param[in] data Memory buffer containing the frame to check
 * @param[in] len  Length of the respBuffer including 2 bytes for CRC
 *
 * @return is the buffer check sum valid
*/
bool PZEM017v2::checkCRC(const uint8_t *buf, uint16_t len){
    if(len <= 2) // Sanity check
        return false;

    uint16_t crc = CRC16(buf, len - 2); // Compute CRC of data
    return ((uint16_t)buf[len-2]  | (uint16_t)buf[len-1] << 8) == crc;
}


/*!
 * PZEM017v2::setCRC
 *
 * Set last two bytes of buffer to CRC16 of the buffer up to byte len-2
 * Buffer must be able to hold at least 3 bytes
 *
 * @param[out] data Memory buffer containing the frame to checksum and write CRC to
 * @param[in] len  Length of the respBuffer including 2 bytes for CRC
 *
*/
void PZEM017v2::setCRC(uint8_t *buf, uint16_t len){ 
    if(len <= 2) // Sanity check
        return;

    uint16_t crc = CRC16(buf, len - 2); // CRC of data

    // Write high and low byte to last two positions
    buf[len - 2] = crc & 0xFF; // Low byte first
    buf[len - 1] = (crc >> 8) & 0xFF; // High byte second
}


// Pre computed CRC table
static const uint16_t crcTable[] PROGMEM = {
    0X0000, 0XC0C1, 0XC181, 0X0140, 0XC301, 0X03C0, 0X0280, 0XC241,
    0XC601, 0X06C0, 0X0780, 0XC741, 0X0500, 0XC5C1, 0XC481, 0X0440,
    0XCC01, 0X0CC0, 0X0D80, 0XCD41, 0X0F00, 0XCFC1, 0XCE81, 0X0E40,
    0X0A00, 0XCAC1, 0XCB81, 0X0B40, 0XC901, 0X09C0, 0X0880, 0XC841,
    0XD801, 0X18C0, 0X1980, 0XD941, 0X1B00, 0XDBC1, 0XDA81, 0X1A40,
    0X1E00, 0XDEC1, 0XDF81, 0X1F40, 0XDD01, 0X1DC0, 0X1C80, 0XDC41,
    0X1400, 0XD4C1, 0XD581, 0X1540, 0XD701, 0X17C0, 0X1680, 0XD641,
    0XD201, 0X12C0, 0X1380, 0XD341, 0X1100, 0XD1C1, 0XD081, 0X1040,
    0XF001, 0X30C0, 0X3180, 0XF141, 0X3300, 0XF3C1, 0XF281, 0X3240,
    0X3600, 0XF6C1, 0XF781, 0X3740, 0XF501, 0X35C0, 0X3480, 0XF441,
    0X3C00, 0XFCC1, 0XFD81, 0X3D40, 0XFF01, 0X3FC0, 0X3E80, 0XFE41,
    0XFA01, 0X3AC0, 0X3B80, 0XFB41, 0X3900, 0XF9C1, 0XF881, 0X3840,
    0X2800, 0XE8C1, 0XE981, 0X2940, 0XEB01, 0X2BC0, 0X2A80, 0XEA41,
    0XEE01, 0X2EC0, 0X2F80, 0XEF41, 0X2D00, 0XEDC1, 0XEC81, 0X2C40,
    0XE401, 0X24C0, 0X2580, 0XE541, 0X2700, 0XE7C1, 0XE681, 0X2640,
    0X2200, 0XE2C1, 0XE381, 0X2340, 0XE101, 0X21C0, 0X2080, 0XE041,
    0XA001, 0X60C0, 0X6180, 0XA141, 0X6300, 0XA3C1, 0XA281, 0X6240,
    0X6600, 0XA6C1, 0XA781, 0X6740, 0XA501, 0X65C0, 0X6480, 0XA441,
    0X6C00, 0XACC1, 0XAD81, 0X6D40, 0XAF01, 0X6FC0, 0X6E80, 0XAE41,
    0XAA01, 0X6AC0, 0X6B80, 0XAB41, 0X6900, 0XA9C1, 0XA881, 0X6840,
    0X7800, 0XB8C1, 0XB981, 0X7940, 0XBB01, 0X7BC0, 0X7A80, 0XBA41,
    0XBE01, 0X7EC0, 0X7F80, 0XBF41, 0X7D00, 0XBDC1, 0XBC81, 0X7C40,
    0XB401, 0X74C0, 0X7580, 0XB541, 0X7700, 0XB7C1, 0XB681, 0X7640,
    0X7200, 0XB2C1, 0XB381, 0X7340, 0XB101, 0X71C0, 0X7080, 0XB041,
    0X5000, 0X90C1, 0X9181, 0X5140, 0X9301, 0X53C0, 0X5280, 0X9241,
    0X9601, 0X56C0, 0X5780, 0X9741, 0X5500, 0X95C1, 0X9481, 0X5440,
    0X9C01, 0X5CC0, 0X5D80, 0X9D41, 0X5F00, 0X9FC1, 0X9E81, 0X5E40,
    0X5A00, 0X9AC1, 0X9B81, 0X5B40, 0X9901, 0X59C0, 0X5880, 0X9841,
    0X8801, 0X48C0, 0X4980, 0X8941, 0X4B00, 0X8BC1, 0X8A81, 0X4A40,
    0X4E00, 0X8EC1, 0X8F81, 0X4F40, 0X8D01, 0X4DC0, 0X4C80, 0X8C41,
    0X4400, 0X84C1, 0X8581, 0X4540, 0X8701, 0X47C0, 0X4680, 0X8641,
    0X8201, 0X42C0, 0X4380, 0X8341, 0X4100, 0X81C1, 0X8081, 0X4040
};


/*!
 * PZEM017v2::CRC16
 *
 * Calculate the CRC16-Modbus for a buffer
 * Based on https://www.modbustools.com/modbus_crc16.html
 *
 * @param[in] data Memory buffer containing the data to checksum
 * @param[in] len  Length of the respBuffer
 *
 * @return Calculated CRC
*/
uint16_t PZEM017v2::CRC16(const uint8_t *data, uint16_t len) 
{
    uint8_t nTemp; // CRC table index
    uint16_t crc = 0xFFFF; // Default value

    while (len--)
    {
        nTemp = *data++ ^ crc;
        crc >>= 8;
        crc ^= (uint16_t)pgm_read_word(&crcTable[nTemp]);
    }
    return crc;
}

/*!
 * PZEM017v2::search
 *
 * Search for available devices. This should be used only for debugging!
 * Prints any found device addresses on the bus.
 * Can be disabled by defining PZEM017v2T_DISABLE_SEARCH
*/
void PZEM017v2::search(){
#if ( not defined(PZEM017v2T_DISABLE_SEARCH))
    static uint8_t response[7];
    for(uint16_t addr = 0x01; addr <= 0xF8; addr++){
        //Serial.println(addr);
        sendCmd8(CMD_RIR, 0x00, 0x01, false, addr);

        if(receive(response, 7) != 7){ // Something went wrong
            continue;
        } else {

            Serial.print("Device on addr: ");
            Serial.println(addr);
        }
    }
#endif
}
